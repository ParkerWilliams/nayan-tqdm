...
class TestFish:
    ...
    def test_is_animation(self, tier: str) -> None:
        assert isinstance(get_theme("fish", tier), Animation)

    @pytest.mark.parametrize("tier", TIERS)
    def test_mode_is_walk(self, tier: str) -> None:
        assert get_theme("fish", tier).mode == AnimationMode.WALK

    @pytest.mark.parametrize("tier", TIERS)
    def test_has_frames(self, tier: str) -> None:
        assert get_theme("fish", tier).frame_count >= 2

    @pytest.mark.parametrize("tier", TIERS)
    def test_fps_positive(self, tier: str) -> None:
        assert get_theme("fish", tier).fps > 0

    @pytest.mark.parametrize("tier", TIERS)
    def test_height_is_3(self, tier: str) -> None:
        anim = get_theme("fish", tier)
    ...
    def test_has_completion(self, tier: str) -> None:
        assert get_theme("fish", tier).completion_frame is not None

    @pytest.mark.parametrize("tier", TIERS)
    def test_completion_height_3(self, tier: str) -> None:
        assert get_theme("fish", tier).completion_frame.height == 3
