...
from .formatters import EMA, format_meter, format_stats_left, format_stats_right
from .fallback import should_use_fallback, render_fallback_bar
from .engine import render_animation, render_themed_bar
from .renderer import render_frame, erase_lines
from .registry import get_theme, get_default_theme, resolve_render_tier
import nayan_tqdm.themes  # noqa: F401 -- trigger built-in theme registration
...
class NyanBar:
    ...
    **kwargs:
    ...
    def _render_animation_frame(self) -> None:
        ...
        # Themed bar path: fill IS the bar
        if self._animation.bar_fill is not None:
            stats_left = format_stats_left(
                self.n, self.total, desc=self._desc,
                unit_scale=self.unit_scale,
            )
            stats_right = format_stats_right(
                self.n, self.total, elapsed, rate,
                unit=self.unit, postfix_str=self._postfix_str,
                unit_scale=self.unit_scale,
            )
            composed = render_themed_bar(
                self._animation, progress, ncols, elapsed,
                stats_left, stats_right,
            )
            if composed:
                self._prev_height = render_frame(
                    composed, self._prev_height, self.file,
                )
            return

        # Legacy path: sprite lines above stats bar
